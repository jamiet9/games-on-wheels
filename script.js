...
class Wheel {
  constructor(opts) {
    ...
    this.getVotingEnabled = opts.getVotingEnabled;
  ...
  draw() {
    ...
    const cx = width / 2;
    const cy = height / 2;
    const radius = Math.max(Math.min(width, height) / 2 - 6, 10);
...
function setupWheels() {
  ...
  wheelsInitialised = true;
}

function ensureWheelsInitialised() {
  if (!wheelsInitialised) {
    setupWheels();
  }
}
...
function enterWheelMode() {
  ensureWheelsInitialised();
  showScreen("wheel");
  ensureMusicStarted();
  if (gameWheel) gameWheel.refresh();
  if (challengeWheel) {
